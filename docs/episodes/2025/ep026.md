...
date:
  created: 2025-11-03
title: "Episode 26 - Super Smooth with Liz Clark"
---
...
[Full transcript available here](../../transcripts/2025/026-transcript.md).

## Listen to the podcast

<div style="width: 100%; height: 200px; margin-bottom: 20px; border-radius: 6px; overflow: hidden;"><iframe style="width: 100%; height: 200px;" frameborder="no" scrolling="no" allow="clipboard-write" seamless src="https://player.captivate.fm/episode/2bffc797-4186-4df1-8c00-9d0de0c3e528/"></iframe></div>

## Show Notes

### 00:20 Stickers!
* [Request a sticker](https://forms.gle/tSsyyREgjA38pUgv6)

### 00:31 Welcome Liz Clark

### 00:53 Liz's Pick #1: Raspberry Pi Connect
...
### 4:24 Paul's Pick #1: OpenBook Touch
...
### 8:39 Tod's Pick #1: atopile: design KiCad circuits with code
...
between the parts of your circuit. From the schematic, you can then create the PCB board layout.
This is the physical layout of exactly which components you'll be using placed exactly where you want them.
For instance, a schematic may have noted a "500 ohm resistor",
but it's during PCB layout when you choose exactly which of the
literally thousands of 500 ohm resistors your circuit needs.

At the end of this design process, you have two drawings:
the schematic for the logical design, the PCB for the physical layout.
...
components and connectivity between them. In a few lines of code you can describe common sub-circuits
like "LED status light", "keyboard matrix", "voltage regulator", or even
"RP2040 micrcontroller".  Each of these sub-circuits becomes re-usable functions in atopile.

Atopile is also a package repository. Sort of like npmjs or pypi, but the modules it knows
...
Most importantly, these atopile modules contain KiCad PCB layout information.
This is especially useful for complicated sub-circuits. For instance, the "rp2040" package
contains the entire KiCad PCB layout for an RP2040 microcontroller circuit,
like what's on the Raspberry Pi Pico.  The somewhat tricky power trace routing and
the correct crystal oscillator and capacitors, critical to the chips function, are all
specified here. Having those tiny but important details bundled up is really nice.

Atopile is also a compiler. Once you have your atopile code written, using these modules,
the atopile compiler generates the PCB.
When you open the board in KiCad, you'll see sub-modules are fully routed,
but the connections between them are not. In theory you'd quickly hand-route these connections
...
Relatedly (at least to me), KiCad supports "Design Blocks" in its schematic view, which are sort of like
the re-usable modules of atopile, but it doesn't yet have the corresponding PCB layout blocks
that atopile has. (There are work-arounds for this but it'd be nice if KiCad had this)
...
### 16:32 Liz's Pick #2: MIDI Baby Gen4
...
* [GitHub repository](https://github.com/DisasterAreaDesigns/MIDI-Baby-Gen4)
* [Midi Baby Gen 4](https://www.disasterareadesigns.com/shop/p/midi-baby)

### 18:44 Paul's Pick #2: GameTank

The GameTank is an open source platform, that's both a game console and a software SDK for writing games. It’s marketed as an open source 8-bit retro console that YOU can build and build games for. It compares itself to fantasy consoles like the Pico-8 or TIC-80, but sets itself apart because it’s a physical hardware device first, followed by an emulator second.  And yes, there’s an emulator also to make development easier.  The emulator runs on Linux, macOS, and Windows.
...
### 21:49 Tod's Pick #2: Tiles from Bergsonne Labs

You know Qwiic/StemmaQT, the cabling standard by SparkFun & Adafruit. It enables
hundreds of different I2C devices to connect in a consistent way,
making it easier for new people and generally making prototyping ideas faster.
But using it makes your project bulky fast. What if there was a micro-sized similar standard?
There may be! It's called Tiles from Bergsonne Labs.  Tiles are only 4mm on a side!
