...
have I got a treat for you.  Short version: use `rpi-clone`.
...
I knew the "real" solution was to run the Pi off an external SSD drive of some kind,
but that seemed pretty daunting, needing extra components like an NVME hat
...
where he off-handedly show off the `rpi-clone` script he uses to turn a Raspbery Pi running
off into one that boots and runs off external storage. Just type `sudo rpi-clone`
into the Pi with the new drive attached and wait a bit.
Then power-down, remove the microSD, and it boots off the new drive.

The best bit to me is there are these small USB 3.1 thumbdrives that are super fast.
I used a $15 Samsung 128GB one that is just a nubbin when plugged in. It's now my Pi's
boot drive using `rpi-clone` and it feels like a whole new computer. Like I got
a free CPU upgrade.  And to prevent this thumdrive from degrading faster, I'm going
to be putting the logs directory into a RAM disk using `log2ram`. It's the constantly
writing of log files that wears out solid-state storage the fastest.
