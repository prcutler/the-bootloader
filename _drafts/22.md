...
play these all simultaneously, since it multi-timbral, with different sounds
per MIDI channel, select with Program Change messages.
...
The SAM2695 is from a French company called Dream.fr, who seems to specialize in one-chip
solutions to many "music hobbyist" needs like "keyboard synthesizer" on a chip,
"USB audio interface", and "81 voice piano chip".
...
Since I have reasons for work to get into them I started the delve, thinking it
would be similar to other ARM-based MCUs I've worked with.

I quickly find that the majority of those are using ST's collection of IDE tools.
Oh no. You know what's worse than trying to use Arduino to write microcontroller code?
